...
GSEPD_Pairs_Plot <- function(GSEPD, txIDs, GO_Term, GO_P, Seg_P){
  
  fc <- GSEPD$normCounts[txIDs,]
  rownames(fc) <- DisplayName(txIDs)
  ...
  square_dimension = length(txIDs)*1.55+2
  pdf(outfile, width = square_dimension, height = square_dimension)
  pairs( t(fc) , main=GO_Term,
        sub=sprintf("%d Genes / %d Samples / GOSeq p=%f / segregation p=%f",length(txIDs), ncol(fc), GO_P, Seg_P),
        pch=pchs, col=cols)
  dev.off();
